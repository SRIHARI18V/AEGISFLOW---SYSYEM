...
class RuleSet:
    ...
    def evaluate(self, txn: Transaction, vel: Velocity) -> list[RuleHit]:
        ...
        if (
            vel.sub_threshold_count_24h >= 3
            and vel.amount_sum_24h >= self.structuring_report_limit
        ):
            hits.append(RuleHit(
                'R003_STRUCTURING',
                1,
                self.weights['R003_STRUCTURING'],
                {
        ...
        if txn.beneficiary_first_seen_at is not None:
            ...
            if (
                age < timedelta(days=self.new_beneficiary_days)
                and txn.amount >= Decimal('100000')
            ):
                hits.append(RuleHit(
