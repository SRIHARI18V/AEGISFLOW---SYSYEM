...
def test_high_value_fires_at_threshold():
    hits = RuleSet().evaluate(
        txn(amount=Decimal('500000')),
        Velocity()
    )
...
def test_velocity_fires_above_limit_only():
    ...
    assert rs.evaluate(
        txn(),
        Velocity(txn_count_1h=12)
    ) == []

    assert rs.evaluate(
        txn(),
        Velocity(txn_count_1h=13)
    )[0].code == 'R002_VELOCITY_1H'
...
def test_structuring_needs_both_count_and_sum():
    ...
    only_sum = Velocity(
        sub_threshold_count_24h=1,
        amount_sum_24h=Decimal('2000000')
    )

    assert rs.evaluate(txn(), only_sum) == []

    both = Velocity(
        sub_threshold_count_24h=4,
        amount_sum_24h=Decimal('2000000')
    )
...
def test_new_beneficiary_requires_material_amount():
    ...
    small = txn(
        amount=Decimal('5000'),
        beneficiary_first_seen_at=recent
    )
    ...
    assert 'R004_NEW_BENEFICIARY' in [
        h.code for h in rs.evaluate(large, Velocity())
    ]
...
def test_dormant_reactivation():
    ...
    t = txn(
        account_last_active_on=NOW - timedelta(days=200)
    )

    assert 'R006_DORMANT_REACTIVATION' in [
        h.code for h in rs.evaluate(t, Velocity())
    ]
...
def test_score_is_capped_at_100():
    ...
    v = Velocity(
        txn_count_1h=40,
        sub_threshold_count_24h=9,
        amount_sum_24h=Decimal('5000000')
    )

    assert rule_score(
        RuleSet().evaluate(t, v)
    ) == 100
...
def test_dedup_key_is_order_independent_and_hour_bucketed():
    a = dedup_key(
        'A1',
        ['R002_VELOCITY_1H', 'R001_HIGH_VALUE'],
        NOW
    )

    b = dedup_key(
        'A1',
        ['R001_HIGH_VALUE', 'R002_VELOCITY_1H'],
