...
CREATE INDEX IF NOT EXISTS idx_txn_occurred
    ON transactions(occurred_at DESC);

CREATE INDEX IF NOT EXISTS idx_txn_amount
    ON transactions(amount) WHERE amount > 100000;
...
CREATE INDEX IF NOT EXISTS idx_velocity_txn
    ON velocity_snapshots(txn_id);
...
CREATE TABLE IF NOT EXISTS rule_hits (
    ...
    FOREIGN KEY (rule_code, rule_version)
        REFERENCES rule_definitions(rule_code, version)
);

CREATE INDEX IF NOT EXISTS idx_rule_hits_txn
    ON rule_hits(txn_id);

CREATE INDEX IF NOT EXISTS idx_rule_hits_code
    ON rule_hits(rule_code, fired_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_watchlist_name
    ON watchlist_entries(lower(entity_name));
