...
RULES = [
    (
        'R001_HIGH_VALUE',
        1,
        'Single transaction above reporting threshold',
        25,
    ...
    (
        'R002_VELOCITY_1H',
        1,
        'More than N transactions in a rolling hour',
        20,
        '{"max_txn_per_hour": 12}'
    ),
    (
        'R003_STRUCTURING',
        1,
        'Multiple sub-threshold debits summing above the report limit',
        35,
        '{"window_hours": 24, "report_limit": 1000000}'
    ),
    (
        'R004_NEW_BENEFICIARY',
        1,
        'High-value transfer to a beneficiary first seen recently',
        15,
        '{"account_age_days": 7}'
    ),
    (
        'R005_HIGH_RISK_COUNTRY',
        1,
    ...
    (
        'R006_DORMANT_REACTIVATION',
        1,
        'Activity on an account dormant beyond N days',
        20,
        '{"dormancy_days": 180}'
    ),
]
        ...
        for a in range(random.randint(1, 2)):
            cur.execute(
                ...
                (
                    customer_id,
                    f'IN66AEGS{i:07d}{a}',
                    random.choice(['current', 'savings', 'card']),
                    'INR',
                    '2021-06-01',
                    '2026-09-01',
                    'active',
...
def main() -> None:
    ...
    with psycopg.connect(DSN, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                "VALUES "
                "('seed_script', 'SEED', 'database', 'aegisflow', "
                "'{\"milestone\": \"M1\"}')"
            )
    ...
    print(
        'seeded: 6 rules, 3 watchlist entries, 120 customers, '
        '~180 accounts, 5000 transactions'
    )
